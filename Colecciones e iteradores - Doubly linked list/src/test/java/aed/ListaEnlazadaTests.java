...
class ListaEnlazadaTests {
    ...
    void iteradorEsBidireccional(){
        ...
        assertTrue(it.haySiguiente());//42?
        assertFalse(it.hayAnterior());//null?
        assertEquals(42, it.siguiente());// 42 -> it:43
        assertTrue(it.haySiguiente());//43?
        assertEquals(43, it.siguiente());// 43 -> it: 44
        assertTrue(it.hayAnterior());//43?
        assertEquals(43, it.anterior());//43 -> it 43
        assertTrue(it.hayAnterior());// 42?
        assertEquals(42, it.anterior());// 42 -> it:42
        assertFalse(it.hayAnterior());// null?
        assertTrue(it.haySiguiente());// 42?
        assertEquals(42, it.siguiente());//42 -> it:43
        assertTrue(it.hayAnterior());//42?
        ...
        assertEquals(43, it.siguiente());// 43 -> it:44
        assertTrue(it.hayAnterior());// 43?
        assertTrue(it.haySiguiente());// 44
        assertEquals(44, it.siguiente());// 44 -> it: null
        assertFalse(it.haySiguiente());//null
        assertTrue(it.hayAnterior());//44?
        assertEquals(44, it.anterior()); // 44 -> it: 44
    }
